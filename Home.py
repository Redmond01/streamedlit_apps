...
from utils.navigation_config import NAVIGATION_CONFIG
...
def render_home_dashboard() -> None:
    """Renders the executive dashboard overview when Home is selected."""
    render_header(
...
# Dynamically construct st.Page items from NAVIGATION_CONFIG
nav_pages: dict[str, list[st.Page]] = {}
for section, items in NAVIGATION_CONFIG.items():
    nav_pages[section] = []
    for item in items:
        ...
        nav_pages[section].append(page)

# Initialize Streamlit central navigation
pg = st.navigation(nav_pages)

# Render global sidebar elements
render_memory_badge()
